...
PACKAGED_DATA_DIR = RESOURCE_DIR / "data"
...
def resolve_data_dir():
    ...
    for base in [Path.cwd(), APP_DIR, RESOURCE_DIR]:
        ...
        for candidate in [current / "data", current.parent / "data", current.parent.parent / "data"]:
...
def load_note_pdfs():
    ...
    for pdf_path in sorted(NOTES_DIR.glob("*.pdf"), key=sort_key):
        ...
        pdfs.append({
            ...
            "url": url_for("serve_note_pdf", filename=pdf_path.name),
        })
    return pdfs
...
@app.post("/api/productivity/note")
...
@app.post("/api/productivity/delete")
